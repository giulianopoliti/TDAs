...
import org.example.adt.DynamicDictionary;
import org.example.adt.ISet;

import java.util.ArrayList;
import java.util.List;

public class SpecialGraph implements IReflexiveGraph {
    ...
    @Override
    public List<Integer> salientOrReflectiveNodes() {
        List<Integer> result = new ArrayList<>();
        for (int i = 0; i < adjacencyMatrix.length; i++) {
            boolean isSalientOrReflective = true;
            for (int j = 0; j < adjacencyMatrix.length; j++) {
                if (this.adjacencyMatrix[i][j] == 1 && i != j) {
                    isSalientOrReflective = false;
                    break;
                }
            }
            if (isSalientOrReflective) {
                result.add(i);
            }
        }
        return result;
    }
    public void invertEdges (int from, int to) {
        if (from == to) {
            return;
        }
        int weight = this.adjacencyMatrix[from][to];
        this.adjacencyMatrix[from][to] = this.adjacencyMatrix[to][from];
